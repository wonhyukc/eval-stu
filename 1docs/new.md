...
- **1차**: 주차(`wk`) 역순 (최근 주차가 위, 예: 3 → 2 → 1)
- **2차**: 대분류(`Type1`) 오름차순 (`class` → `fin` → `hw` → `mid`)
- **3차**: 세부유형(`Type2`) 오름차순 (`0.1` → `0.2` → `0.3` → `0.p`)
- **4차**: 학번(`ID`) 오름차순 (예: 741 → 742 → 870...)
- **번호(`No`)**: 최상단부터 아래로 내림차순(N down to 1) 순차 부여 (최상단이 최대 번호)
