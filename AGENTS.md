# AGENTS.md — eval-stu 운영 지침 (SSOT)

이 문서는 `eval-stu` 저장소의 **유일한 규칙 원천(SSOT)**입니다. **학생 평가(과제 수집·채점·성적 관리)** 자동화 저장소 운영 지침을 담습니다.

> **⚠️ SSOT 경고**: `.agents/rules/language.md`와 `.agents/rules/rules-project.md`는 **생성 금지**입니다. 모든 규칙은 이 문서(AGENTS.md)에만 정의됩니다. pre-commit 훅이 재생성을 차단합니다.

- 스킬: `.agents/skills/<name>/SKILL.md` (Claude Code에는 `.claude/skills` 심볼릭 링크로 연동)
- 워크플로우: `.agents/workflows/*.md` (Claude Code에는 `.claude/commands` 심볼릭 링크로 연동)
- 프로젝트 개요 및 디렉터리 구조는 `README.md` 참고.

---

## 📚 2026-2학기 개설 강좌 및 운영 분반

**분반 및 과목 동의어 매핑 (Course Synonyms)**

| 대표 명칭 | 동의어 (호환 표기) | 대상 강좌 및 분반 상세 |
|:---:|---|---|
| **1반** | `01`, `web1`, `웹1`, `ICE0001-01`, `761`, `15143` | E트랙 `ICE0001-01` 웹프로그래밍(E트랙) 01분반 (월 16:00\~18:45, 7-9교시) |
| **2반** | `02`, `web2`, `웹2`, `ICE0001-02`, `762`, `15144` | E트랙 `ICE0001-02` 웹프로그래밍(E트랙) 02분반 (월 13:00\~15:50, 5-7교시) |
| **4반** | `04`, `python`, `파이썬`, `GPU0940-04`, `468`, `14712` | K트랙 `GPU0940-04` 컴퓨팅적사고와코딩(K트랙) (월 09:00\~11:50, 1-3교시) |

> **과목 호칭 원칙**: 모든 과목은 대표 명칭(**1반**, **2반**, **4반**) 및 정의된 동의어를 공식 동의어로 취급하며 상호 완벽히 호환하여 인식하고 사용한다.

| 대표 명칭 | 동의어 (호환 표기) | 트랙 / 분반 | 학과명 | 교과목번호 (강좌ID) | 교과목명 | 학점 | 수업시간 | 강의실 | 수강 인원 | 언어 |
|:---:|---|:---:|---|:---:|---|:---:|---|---|:---:|:---:|
| **1반** | `01`, `web1`, `웹1`, `ICE0001-01`, `761`, `15143` | E트랙 01분반 | 컴퓨터공학과(국) | **ICE0001-01** (15143) | 웹프로그래밍(E트랙) | 3 | 월(16:00\~18:45, 7-9교시) | 본관 IT융합소프트웨어학과 실습실 (3층 308호) | 31 | 영어 |
| **2반** | `02`, `web2`, `웹2`, `ICE0001-02`, `762`, `15144` | E트랙 02분반 | 컴퓨터공학과(국) | **ICE0001-02** (15144) | 웹프로그래밍(E트랙) | 3 | 월(13:00\~15:50, 5-7교시) | 본관 IT융합소프트웨어학과 실습실 (3층 308호) | 32 | 영어 |
| **4반** | `04`, `python`, `파이썬`, `GPU0940-04`, `468`, `14712` | K트랙 | 교양교육원 | **GPU0940-04** (14712) | 컴퓨팅적사고와코딩(K트랙) | 3 | 월(09:00\~11:50, 1-3교시) | 본관 컴퓨터실습실 (2층 205호) | 9 | 한국어 |

---

## 🌐 플리토(Flitto Live TR) 실시간 통번역 시스템

외국인 유학생(베트남어, 네팔어, 미얀마어 등)의 수업 지원을 위해 실시간 번역 서비스를 운영합니다.

- **접속 링크**: [Flitto Live TR](https://livetr.flit.to/login)
- **로그인 정보**:
  - 아이디: `stu02.host`
  - 비밀번호: `Stu2026!@#`
- **방 입장**:
  - 4반 (파이썬): `정원혁 / 월 1-3교시 / 컴퓨팅적사고와코딩(K트랙)`
  - 1반 / 2반 (웹): `정원혁 / 월 5-10교시 웹프로그래밍(E트랙)`
- **마이크 수령/반납**: 국제학부 사무실 (우석 1층, 032-340-9578)

---

## 📊 성적 및 학생 관리 (Grade Management)


- **전체 학생 (비공유, 교수 전용)**: [전체 학생(Google Sheets)](https://docs.google.com/spreadsheets/d/1lUdHWhyNDTZl9n7s48jn3FCrH6gcbQvGt1nkLBbIM9o/edit?gid=1889091726#gid=1889091726) *(서비스 계정 미공유, 교수 본인만 열람)*
- **파이썬 4반 성적부**: [파이썬 4반 성적부(Google Sheets)](https://docs.google.com/spreadsheets/d/1Ni4ZaeIJJdNNvysx5-LFpOF6sh4Emp-cK92DnTUsAyQ/edit?gid=1514293361#gid=1514293361)
- **웹프로그래밍 (1반 / 2반) 과제 및 성적부**: [web-stu0926 성적부(Google Sheets)](https://docs.google.com/spreadsheets/d/1OMeWuYt45TZMygmkh5hOhqSCCUFYJv4iE0hTY554iAo/edit?gid=1892167835#gid=1892167835) *(⚠️ 대원칙: 이 시트에 기록할 때는 헤더, 사유, 내용 일체가 항상 100% 영어여야 함)*
- **학생 명단 SSOT**: `5input/students/` 디렉터리 내의 `py-students.md`, `wb-students.md`, `students_summary.md`
- **배포 및 공유 자료 언어 정책 (Distribution & Sheet Language Policy)**:
  - 학생들에게 배포·공유되는 모든 자료(스프레드시트, 과제 피드백, 안내문 등)는 해당 트랙의 강의 언어와 100% 일치하도록 작성한다.
  - **E트랙 (영어 트랙: 1반 / 2반 웹프로그래밍)**: 헤더뿐만 아니라 **Reason(사유), 상태 등 모든 셀의 내용과 텍스트를 예외 없이 100% 영어**로 작성한다. (`No`, `StudentID`, `Track`, `Score`, `Type`, `Reason`, `Date`, `Name`, `Subject`)
  - **K트랙 (한국어 트랙: 4반 파이썬)**: 헤더뿐만 아니라 **이유(사유), 상태 등 모든 셀의 내용과 텍스트를 예외 없이 100% 한국어**로 작성한다. (`no`, `학번`, `트랙`, `점수`, `유형`, `이유`, `날짜`, `이름`, `메일제목`)
- **채점 및 시트 동기화 멱등성 보장 (Idempotency)**:
  - 동일 스크립트를 여러 번 재실행해도 중복 데이터가 누적되지 않고 항상 단일 상태로 수렴해야 한다.
  - 학생 1인이 여러 메일을 발송한 경우 **마감 기한 이전의 가장 최신 메일 1건**을 기준으로 채점한다. (마감 전 제출본이 있으면 지각 처리하지 않음)
  - 구글 시트 연동 시 단순 Append를 금지하고, `(StudentID, Type)` 복합 키 기반의 **Upsert(있으면 갱신, 없으면 추가)**를 적용한다.
  - 구글 시트의 `Type` 열과 `Subject(메일제목)` 열(I열)은 학번 등 순수 숫자가 오인 변환되는 것을 방지하기 위해 **반드시 항상 문자열(String) 타입으로 강제 포맷팅(' 접두사 적용)**하여 입력한다.
- **과제 이메일 본문 유효성 검증 (Pure Body Validation)**:
  - 단순 제목 매칭(`과제0.x 학번`)만으로 채점하지 않는다.
  - 교강사의 이전 안내/답장 인용문(`>`, `wrote:`, `đã viết:`)을 제외한 **학생의 순수 작성 본문(소감, 인증, 링크 등)이 실질적으로 존재**해야 유효 제출로 인정한다.
  - 이전 안내 메일에 제목만 수정하여 회신한 **내용 없는 빈 회신 메일은 정상 제출(1.0점) 처리 및 "잘 받았습니다" 답장 발송을 엄격히 금지**한다.
- **과제 마감 시간**: **매주 목요일 23:59 KST** (과제 0.x의 마감은 해당 주차 목요일 23:59)
- **과제 세부 감점 기준 (Deduction Criteria, 1.0점 만점 기준)**:
  - **마감 시간 이후 제출 (지각)**: **-0.5점**
  - **제목 규칙 위반 (양식/학번 오류, 불필요 문구/대괄호 등 추가)**: **-0.3점**
  - **본문 비어 있음 (인용문만 있거나 내용 누락)**: **-0.2점**
  - 복합 위반 시 감점 합산 적용 (예: 지각 -0.5 + 제목위반 -0.3 = 0.2점)
- **스레드 원본 제목 및 발신자 분리 검증 (Thread Origin Subject Verification)**:
  - 교강사의 수신 확인 또는 피드백 답장으로 인해 붙은 `Re:` 접두사를 학생이 직접 작성한 제목으로 오인하여 형식 위반(경고/감점) 처리하지 않는다.
  - 제목 양식 평가는 스레드 전체의 최신 제목이 아닌, **해당 학생이 직접 발송한 메시지의 최초/최신 원본 제목**을 기준으로 검증한다.

---

## 🔗 형제 저장소와의 경계

### 강의안 저장소 (`../stu2603`)

- 강의안·과제 SSOT는 별도 저장소 `../stu2603`입니다. 이 저장소에서는 **읽기만** 하고, 수정은 stu2603 쪽에서 합니다.
- 교차 참조는 심볼릭 링크로 연결: `1docs/assignment-micro.md`, `1docs/scores.md`, `5input/py`, `5input/web`
- 교차 링크는 **항상 상대경로**로 만듭니다(마운트 경로 변경에도 살아남도록). 깨진 링크는 pre-commit 훅이 차단합니다.

### 과목별 교재 폴더 (`../py2608-stu`, `../web2608-stu`)

- `../py2608-stu` — 파이썬 4반 (K트랙) 교재·과제 자료
- `../web2608-stu` — 웹 1반/2반 (E트랙) 교재·과제 자료
- 이 저장소(`eval-stu`)에서 **채점·성적 기록·이메일 답장 발송**을 수행합니다.

---

## ⚠️ 핵심 제약 조건

- **자격증명 파일 접근 금지**: `secret.json`, `credentials.json`, `token.json`, `service-account.json`은 읽지도, 출력하지도, 커밋하지도 않습니다.
- **학생 개인정보 보호**: 성적 CSV/TSV, 학생 사진, 이메일 본문은 git에 추가하지 않습니다 (`output/`, `9output/`, `*.csv`, `*.tsv`는 gitignore 유지).
- **파이썬 코드 검증**: `.py` 수정 후에는 `./bin/harness-check.sh`(Black·Flake8·Mypy·Pytest)를 통과해야 하며, pre-commit 훅이 강제합니다.
- **출력 폴더 분리**: 출력 폴더는 두 곳입니다: `output/`(채점·점수 CSV), `9output/`(상호평가 배정 md, 등급 산출물). 코드가 각각 참조하므로 임의로 합치지 않습니다.

---

## AI 행동 규칙 (에이전트 전용)

**1. 사전 확인 (작업 시작 전 필독)**

작업 시작 전 이 저장소 루트의 `AGENTS.md`를 반드시 읽는다. 모든 규칙은 이 문서에만 정의되어 있다.

**2. 검증 우선 원칙**

- `.py` 수정 후에는 반드시 `./bin/harness-check.sh`를 실행하여 검증한다.
- 비밀 정보를 포함할 가능성이 있는 파일(`.env`, 인증 정보 등)은 커밋하지 않는다.

**3. 커밋 스테이징**

- 현재 작업에 관련된 파일만 `git add <파일>`로 명시적으로 스테이징한다.
- 비밀 정보를 포함할 가능성이 있는 파일(`.env`, 인증 정보 등)은 커밋하지 않는다.

**4. 임시 스크립트 및 일회성 자동화 파일**

- 임시 또는 일회성 스크립트는 **항상** 프로젝트 루트의 **`tmp/`** 디렉터리에 생성한다.

**5. 언어 정책**

- 채팅 대화, 임시 응답 등 기타 출력은 사용자의 주 사용언어(한국어)로만 작성한다.
