...
def parse_markdown(md_file, track_name, week):
    rows = []
...
def main():
    ...
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    md1 = os.path.join(base_dir, "9output", "week11_peer_review_assignments_761.md")
    md2 = os.path.join(base_dir, "9output", "week11_peer_review_assignments_762.md")

    rows_761 = parse_markdown(md1, "761", "11")
    rows_762 = parse_markdown(md2, "762", "11")

    all_rows = []
    all_rows.extend(rows_761)
    all_rows.extend(rows_762)

    range_name = f"{sheet_title}!A:F"
    ...
    print(f"✅ Appended {updated_cells} cells in {sheet_title}")
