    ...
    if wb_path.exists():
        with open(wb_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if len(cols) > 8 and cols[3].isdigit():
                    sid = cols[3]
                    trk = "1" if cols[1] == "15143" else "2" if cols[1] == "15144" else cols[1]
                    students_by_id[sid] = {
                        "id": sid,
                        "track": trk,
                        "course_id": cols[1],
...
def evaluate_submission(
    ...
    for line in body_lines:
        s = line.strip()
        if s.startswith(">") or s.startswith("wrote:") or "đã viết:" in s or "작성:" in s:
            continue
        pure_lines.append(s)
    ...
    if pure_len < 30 and not has_url:
        return 0.0, "Empty Body / Quoted Text Only", "본문 미작성(단순 회신)", f"Pure body too short ({pure_len} chars) & no URL"
    ...
    if email_dt:
        if email_dt > late_cutoff_dt:
            return 0.0, "Rejected (Past late cutoff)", "불인정 (지각 마감 초과)", f"Submitted after late cutoff ({late_cutoff_dt})"
        if email_dt > deadline_dt:
            is_late = True

    # 3. 제목 검사
    clean_sub = re.sub(r"^(\s*re:\s*|\s*fwd:\s*)+", "", subject, flags=re.IGNORECASE).strip()
    ...
    if not is_late:
        if is_exact:
            return 1.0, "On-time & Exact Format", "정상 제출 (기한내/정확한 양식)", "Exact format"
        elif has_task and has_id:
            return 0.9, "Minor format issue (Brackets/Extra text)", "경미한 양식 오차 (괄호/불필요 기호)", "Brackets, labels or extra text"
        else:
            return 0.7, "Missing Student ID or 0.x in Subject", "제목 학번 또는 과제명 누락", "Missing ID or assignment number"
    else:
        if has_id and has_task:
            return 0.5, "Late submission (Before next class)", "지각 제출 (다음 수업 시작 전)", "Late but ID present"
        else:
            return 0.2, "Late & Format Issue (Missing ID)", "지각 + 제목 학번 누락", "Late & format issue"
...
def send_ntfy(title: str, body: str, tags: str = "mortar_board", priority: str = "default"):
    """ntfy.sh 푸시 알림 발송."""
...
def run_grading(
    ...
    if not ensure_chrome_running(log):
        ...
        send_ntfy("❌ 채점 실패", f"과제 {task_id}: Chrome 기동 실패", tags="rotating_light", priority="high")
        return
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                ...
                send_ntfy("❌ 채점 실패", f"과제 {task_id}: Gmail 로그인 필요", tags="rotating_light", priority="high")
                return
            ...
            search_query = f'("{task_num}" OR "과제 {task_num}" OR "assignment {task_num}") after:{search_after} -from:comments-noreply@docs.google.com'
            ...
            if row_count == 0:
                page.fill('input[name="q"]', f"{task_num} after:{search_after} -from:comments-noreply@docs.google.com")
                page.keyboard.press("Enter")
                page.wait_for_timeout(5000)
                email_rows = page.locator("table.F.cf.zt:visible tr.zA:visible")
                row_count = email_rows.count()
            ...
            for idx in range(row_count):
                ...
                sender_name = (sender_el.first.get_attribute("name") or sender_el.first.inner_text()) if sender_el.count() > 0 else ""

                date_el = target_row.locator("td.xW span")
                date_str = date_el.first.get_attribute("title") if date_el.count() > 0 else ""
                ...
                log.info(f"   메시지 수: {msg_count}, 제목: '{subject}', 발신자: '{sender_name}'")
                ...
                for m_idx in range(msg_count):
                    ...
                    s_email = s_email_el.get_attribute("email") if s_email_el.count() > 0 else ""
                    ...
                    if m_idx == msg_count - 1:
                        ...
                        if d_el.count() > 0:
                            last_msg_date_str = d_el.first.get_attribute("title") or d_el.first.inner_text()
                ...
                student_info = students_by_id.get(extracted_sid) if extracted_sid else None

                # Grace-period 모드: 해당 트랙만 처리
                if grace_track and student_info and student_info["track"] != grace_track:
                    log.info(f"   ⏭️ Grace-period 모드: 트랙 {student_info['track']} → 스킵 (대상: {grace_track})")
                    stats["skipped"] += 1
                    back_btn = page.locator('div[aria-label="Back to search"], div[aria-label="검색결과로 돌아가기"], div[act="19"]')
                ...
                if not student_info:
                    log.warning(f"   ⚠️ 수강생 매칭 실패 (Subject: {subject}, Sender: {sender_name})")
                else:
                    log.info(f"   👤 매칭: {student_info['eng_name']} ({student_info['id']}, 트랙{student_info['track']})")
                ...
                log.info(f"   ✉️ 답장: {'완료(스킵)' if is_replied else '미답장(대상)'}")
                stats["graded"] += 1
            ...
            if not dry_run:
                ...
                for item in final_list:
                    ...
                    if s_info["track"] == "4":
                        row = [
                            0, wk_num, s_info["id"], "4",
                            f"{item['score']:.2f}", "hw", task_id,
                            item["reason_ko"], item["date"],
                            s_info["eng_name"], item["subject"],
                        ]
                        py_rows.append(row)
                    else:
                        row = [
                            0, wk_num, s_info["id"], s_info["track"],
                            f"{item['score']:.2f}", "hw", task_id,
                            item["reason_en"], item["date"],
                            s_info["eng_name"], item["subject"],
                        ]
                        web_rows.append(row)
            ...
            if not dry_run:
                ...
                for idx, item in enumerate(unreplied):
                    s_info = item["student_info"]
                    log.info(f"   → [{idx+1}/{len(unreplied)}] {s_info['eng_name']} ({s_info['id']})")
...
def main():
    ...
    parser.add_argument("--task", type=str, help="과제 번호 (예: 0.3). 미지정 시 자동 계산")
    parser.add_argument("--grace", type=str, choices=["py", "web1", "web2"],
                        help="Grace-period 마감 모드 (해당 트랙만 처리)")
    parser.add_argument("--dry-run", action="store_true", help="시트/답장 없이 채점만 미리보기")
    ...
    log = setup_logging(task_id)
    log.info(f"🎓 auto_grade_email.py 시작 (task={task_id}, grace={args.grace}, dry_run={args.dry_run})")
