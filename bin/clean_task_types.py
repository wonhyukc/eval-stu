...
from modules.sheet_updater import get_sheet_service, load_course_config, get_target_sheet_title
...
def clean_sheet(course):
    ...
    for row_idx, row in enumerate(values[1:], start=2): # 1-based index, row 1 is header
        if len(row) > 4:
            original_val = str(row[4])
            clean_val = original_val.strip().replace("과제", "").replace("assignment", "").strip()
            
            # 비정상 데이터 식별 (날짜 포맷이나 의미 없는 문자열 등)
            if "finder" in clean_val.lower() or "/" in clean_val:
                print(f"⚠️ [수동확인 요망] 행 {row_idx}: (학번: {row[1] if len(row)>1 else '?'}) | 비정상 유형 값 발견: '{original_val}'")
            
            # 구글 시트에 문자열(Text)로 강제 지정되도록 ' 접두어 추가
            target_val = f"'{clean_val}"
            
            updates.append({
                "range": f"{sheet_title}!E{row_idx}",
                "values": [[target_val]]
            })
    ...
    if updates:
        ...
        print(f"✅ [{course.upper()}] 과정 E컬럼 포맷팅 완료! (총 {resp.get('totalUpdatedCells')}개 셀 강제 문자열 변환 적용)")
    else:
